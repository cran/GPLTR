...
{ 
  ...
  Vect_CV_ERROR0 <- mean(CV_ERROR0)
  SIZES <- sapply(CV_ERRORS,length)
  SIZE <- min(SIZES)
  CV_ERRORS <- lapply(CV_ERRORS, function(u) u[1:(SIZE)])
  vect_CV_ERRORS <- apply(matrix(unlist(CV_ERRORS), ncol=ncv, byrow = FALSE), 1, mean)
  CV_ERRORS <- c(Vect_CV_ERROR0,vect_CV_ERRORS)
  CV_ERROR <- min(CV_ERRORS)
  best_index_CV <- which.min(CV_ERRORS)
  nested_tree = nested.trees(xtree, xdata, Y.name, X.names, G.names,MaxTreeSize = MaxTreeSize, family = family, verbose = verbose)
