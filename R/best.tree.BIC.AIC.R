best.tree.BIC.AIC <- function(xtree, xdata, Y.name, X.names, G.names, family = "binomial", verbose = TRUE)
{
  ...
  else lm_BIC = glm(as.formula(paste(Y.name, "~", paste(X.names, collapse = " + "))), data = xdata, family = family)
  ...
  else lm_AIC = glm(as.formula(paste(Y.name, "~", paste(X.names, collapse = " + "))), data = xdata, family = family)
